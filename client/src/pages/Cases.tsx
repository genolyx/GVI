    ...
    <div className="space-y-7">
      ...
      <div className="clinical-card overflow-hidden">
        ...
        <div className="hidden overflow-x-auto sm:block">
          {query.isLoading ? <div className="space-y-3 p-5">{Array.from({ length: 6 }).map((_, index) => <Skeleton key={index} className="h-12" />)}</div> : query.data?.length ? <table className="w-full min-w-[820px] text-left"><thead><tr className="border-b border-border/70 bg-muted/35 text-[10px] uppercase tracking-wider text-muted-foreground"><th className="px-5 py-3 font-semibold">Case</th><th className="px-5 py-3 font-semibold">Project</th><th className="px-5 py-3 font-semibold">Purpose</th><th className="px-5 py-3 font-semibold">Input</th><th className="px-5 py-3 font-semibold">Build / Panel</th><th className="px-5 py-3 font-semibold">Status</th><th className="px-5 py-3 font-semibold">Updated</th></tr></thead><tbody className="divide-y divide-border/60">{query.data.map(item => <tr key={item.id} tabIndex={0} role="link" aria-label={`Open case ${item.caseNumber}`} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); navigate(`/cases/${item.id}`); } }} onClick={() => navigate(`/cases/${item.id}`)} className="cursor-pointer hover:bg-muted/45 focus-visible:bg-primary/[0.055] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-primary"><td className="px-5 py-3.5"><p className="font-mono text-xs font-medium">{item.caseNumber}</p><p className="mt-1 text-[11px] text-muted-foreground">{item.patientAlias}</p></td><td className="px-5 py-3.5 text-xs">{item.projectName}</td><td className="px-5 py-3.5"><span className={`inline-flex items-center gap-2 text-xs capitalize ${item.purpose === "germline" ? "text-teal-700" : "text-violet-700"}`}><span className={`size-1.5 rounded-full ${item.purpose === "germline" ? "bg-teal-500" : "bg-violet-500"}`} />{item.purpose}</span></td><td className="px-5 py-3.5 font-mono text-[11px] uppercase">{item.inputType}</td><td className="px-5 py-3.5"><p className="text-xs">{item.referenceBuild}</p><p className="mt-1 max-w-[180px] truncate text-[10px] text-muted-foreground">{item.panelName || "—"}</p></td><td className="px-5 py-3.5"><ClinicalStatus status={item.status} /></td><td className="px-5 py-3.5 text-[11px] text-muted-foreground">{new Date(item.updatedAt).toLocaleString()}</td></tr>)}</tbody></table> : emptyState}
        </div>
      </div>
    </div>
  );

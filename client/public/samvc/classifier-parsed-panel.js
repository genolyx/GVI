...
if (res.parsed_data.cryptic_inserted_cdna) {
    ...
    junctionInsertHtml = `<div style="padding: 8px 12px; background: rgba(236, 253, 245, 0.06); border-left: 3px solid #34d399; border-radius: 4px; font-size: 0.85em; color: #a7f3d0; line-height: 1.4;"><strong style="color:#6ee7b7;">Junction insert (resolved cDNA):</strong> <code style="color:#ecfdf5;">${_jShow}</code> <span style="color:#94a3b8;">(${_jIns.length} bp${_jIns.length > 20 ? ', first 20 shown' : ''})</span>${_jGain}</div>`;
}
const nmdNarrativeOnly = stripNmdMathCatalogue(res.parsed_data.nmd_math);
const nmdNarrativeHtml = nmdNarrativeOnly
    ? `<div style="padding: 10px; background: rgba(255, 255, 255, 0.05); border-left: 3px solid #fca5a5; border-radius: 4px; font-size: 0.9em; color: #e2e8f0; line-height: 1.4;"><strong>Truncation &amp; NMD math:</strong> ${nmdNarrativeOnly}</div>`
    : '';
...
${res.parsed_data.spliceai_narrative ? `<div style="padding: 10px 12px; background: rgba(56, 189, 248, 0.08); border-left: 3px solid #38bdf8; border-radius: 4px; font-size: 0.9em; color: #e0f2fe; line-height: 1.45;"><strong style="color:#7dd3fc;">SpliceAI summary:</strong> ${res.parsed_data.spliceai_narrative.replace(/</g, '&lt;')}</div>` : ''}
${((res.parsed_data.spliceai_junction_model_preferred || res.parsed_data.spliceai_competing_splice_isoforms) && res.parsed_data.splice_model_interpretation) ? `<div style="padding: 10px 12px; background: rgba(16, 185, 129, 0.08); border-left: 3px solid #34d399; border-radius: 4px; font-size: 0.88em; color: #d1fae5; line-height: 1.45;"><strong style="color:#6ee7b7;">Splice model:</strong> ${String(res.parsed_data.splice_model_interpretation).replace(/</g, '&lt;')}</div>` : ''}
${junctionInsertHtml}
${(res.parsed_data.nmd_junction_model_truncation_fraction != null && res.parsed_data.nmd_junction_model_truncation_fraction !== undefined && !res.parsed_data.splice_suppress_parallel_junction_ui) ? `<div style="padding: 8px 12px; background: rgba(59, 130, 246, 0.08); border-left: 3px solid #3b82f6; border-radius: 4px; font-size: 0.85em; color: #dbeafe; line-height: 1.4;"><strong style="color:#93c5fd;">Cryptic junction isoform</strong> (acceptor/donor gain product): &approx; <strong>${(100 * res.parsed_data.nmd_junction_model_truncation_fraction).toFixed(1)}%</strong> C-term lost${res.parsed_data.junction_model_hgvs_p ? ` — ${res.parsed_data.junction_model_hgvs_p}` : ''}. <span style="color:#94a3b8;">Parallel to whole-exon skip when SpliceAI shows competing gain vs loss.</span></div>` : ''}
${res.parsed_data.splice_products_panel_html ? `<div style="padding: 10px 12px; background: rgba(255, 255, 255, 0.05); border-left: 3px solid #60a5fa; border-radius: 4px; font-size: 0.9em; color: #e2e8f0; line-height: 1.5;"><strong style="color:#93c5fd;">Splice products</strong> <span style="color:#94a3b8;font-size:0.82em;display:block;margin:4px 0 10px;">Each product uses the same field order: SpliceAI signal → geometry → frame → protein → PTC → length → NMD.</span>${res.parsed_data.splice_products_panel_html}</div>` : ''}
${!res.parsed_data.splice_products_panel_html && res.parsed_data.splice_frame_math ? `<div style="padding: 10px; background: rgba(255, 255, 255, 0.05); border-left: 3px solid #60a5fa; border-radius: 4px; font-size: 0.9em; color: #e2e8f0; line-height: 1.4;"><strong>Splice products:</strong> ${res.parsed_data.splice_frame_math}</div>` : ''}
${!res.parsed_data.splice_products_panel_html && res.parsed_data.spliceai_secondary_splice_frame_math ? `<div style="padding: 10px; background: rgba(251, 191, 36, 0.07); border-left: 3px solid #fbbf24; border-radius: 4px; font-size: 0.9em; color: #fef3c7; line-height: 1.45;"><strong style="color:#fcd34d;">Parallel whole-exon skip (second splice-site hypothesis)</strong>${res.parsed_data.spliceai_secondary_splice_frame_math}</div>` : ''}
${!res.parsed_data.splice_products_panel_html && res.parsed_data.deep_intronic_primary_splice_math ? `<div style="padding: 10px; background: rgba(16, 185, 129, 0.08); border-left: 3px solid #34d399; border-radius: 4px; font-size: 0.9em; color: #d1fae5; line-height: 1.45;"><strong style="color:#6ee7b7;">Splice products — primary</strong><div style="margin-top:8px;">${res.parsed_data.deep_intronic_primary_splice_math}</div></div>` : ''}
...
${spliceMapHtml}
${junctionAlignHtml}
${res.parsed_data.splice_math_error ? `<div style="padding: 10px; background: rgba(245, 158, 11, 0.1); border-left: 3px solid #f59e0b; border-radius: 4px; font-size: 0.9em; color: #f59e0b; line-height: 1.4;"><strong>Splice Structural Math:</strong> ${res.parsed_data.splice_math_error}</div>` : ''}
  </div>
</section>
...
<section class="eval-section">
  ...
  <div class="eval-substack">
${nmdNarrativeHtml}
${res.parsed_data.nmd_math_error ? `<div style="padding: 10px; background: rgba(245, 158, 11, 0.1); border-left: 3px solid #f59e0b; border-radius: 4px; font-size: 0.9em; color: #f59e0b; line-height: 1.4;"><strong>NMD Structural Math:</strong> ${res.parsed_data.nmd_math_error}</div>` : ''}
${res.parsed_data.deep_intronic_splice_html ? `<div style="padding: 10px; background: rgba(255, 255, 255, 0.05); border-left: 3px solid #a78bfa; border-radius: 4px; font-size: 0.9em; color: #e2e8f0; line-height: 1.45;"><strong>Deep intronic splice scan:</strong> ${res.parsed_data.deep_intronic_splice_html}</div>` : ''}
  </div>
</section>` : ''}
